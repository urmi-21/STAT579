...
ggplot(data=avgs, aes(x=avg))+geom_bar() + facet_wrap(~SEX)
ggplot(data=avgs, aes(x=avg))+geom_bar() + facet_wrap(SEX~DEGREE)

```

The plot looks different for different conbinations of variables. The min avg is small for female compared to men. Male with high school have high average vs male with graduate or bachelor. Female with bachelor or graduate has less avg the female with high school degree.